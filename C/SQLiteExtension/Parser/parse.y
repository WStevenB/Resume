...
values(A) ::= VALUES LP nexprlist(X) RP. {
  A = sqlite3SelectNew(pParse,X,0,0,0,0,0,0,SF_Values,0,0);
}
values(A) ::= values(A) COMMA LP exprlist(Y) RP. {
  Select *pRight, *pLeft = A;
  pRight = sqlite3SelectNew(pParse,Y,0,0,0,0,0,0,SF_Values|SF_MultiValue,0,0);
...
%ifndef SQLITE_OMIT_SUBQUERY
  ...
  seltablist(A) ::= stl_prefix(A) LP seltablist(F) RP
    ...
    if( A==0 && Z.n==0 && N==0 && U==0 ){
      ...
      pSubquery = sqlite3SelectNew(pParse,0,F,0,0,0,0,0,SF_NestedFrom,0,0);
